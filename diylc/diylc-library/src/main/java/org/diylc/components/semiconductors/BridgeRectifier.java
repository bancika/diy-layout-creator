...
@ComponentDescriptor(name = "Bridge Rectifier", author = "Branislav Stojkovic",
...
public class BridgeRectifier extends AbstractLabeledComponent<String> implements IGerberComponentSimple {
  ...
  public Area[] getBody() {
    if (body == null) {
      ...
      double centerX = (controlPoints[0].getX() + controlPoints[1].getX() + controlPoints[2].getX()
          + controlPoints[3].getX()) / 4;
      double centerY = (controlPoints[0].getY() + controlPoints[1].getY() + controlPoints[2].getY()
          + controlPoints[3].getY()) / 4;
      ...
      switch (rectifierType) {
        ...
        case MiniDIP2:
        case InLine:
          body[0] = new Area(new RoundRectangle2D.Double(centerX - length / 2, centerY - width / 2,
              length, width, EDGE_RADIUS, EDGE_RADIUS));
          break;
        case MiniRound1:
        case MiniRound2:
          body[0] = new Area(
        ...
        case SquareBR3:
          ...
          RoundedPolygon poly =
              new RoundedPolygon(new Point2D[] {new Point2D.Double(centerX, centerY - width / 2),
              ...
              }, new double[] {EDGE_RADIUS, EDGE_RADIUS, EDGE_RADIUS, EDGE_RADIUS / 2,
                  EDGE_RADIUS / 2});

          body[0] = new Area(poly);
          body[0].subtract(new Area(new Ellipse2D.Double(centerX - holeSize / 2,
              centerY - holeSize / 2, holeSize, holeSize)));
  ...
  public void draw(Graphics2D g2d, ComponentState componentState, boolean outlineMode,
    ...
    int pinSize = (int) (pinShape == PinShape.Round ? ROUND_PIN_SIZE.convertToPixels()
        : SQUARE_PIN_SIZE.convertToPixels()) / 2 * 2;
    ...
    if (!outlineMode) {
      for (Point2D point : controlPoints) {
        ...
        if (pinShape == PinShape.Round)
          shape = new Ellipse2D.Double(point.getX() - pinSize / 2, point.getY() - pinSize / 2,
              pinSize, pinSize);
        else
          shape = new Rectangle2D.Double(point.getX() - pinSize / 2, point.getY() - pinSize / 2,
              pinSize, pinSize);
        g2d.fill(shape);
    ...
    for (int i = 0; i < controlPoints.length; i++) {
      ...
      StringUtils.drawCenteredText(g2d, pointLabels[i], point.getX() + dx, point.getY() + dy,
  ...
  public void drawIcon(Graphics2D g2d, int width, int height) {
    ...
    StringUtils.drawCenteredText(g2d, "+", width * 2 / 8 + 1, height * 2 / 8 + 4 * width / 32,
        HorizontalAlignment.CENTER, VerticalAlignment.CENTER);
    StringUtils.drawCenteredText(g2d, "-", width * 2 / 8 + 1, height * 6 / 8 - 5 * width / 32,
        HorizontalAlignment.CENTER, VerticalAlignment.CENTER);
    StringUtils.drawCenteredText(g2d, "~", width * 6 / 8 - 2 * width / 32,
        height * 2 / 8 + 5 * width / 32, HorizontalAlignment.CENTER, VerticalAlignment.CENTER);
    StringUtils.drawCenteredText(g2d, "~", width * 6 / 8 - 2 * width / 32,
        height * 6 / 8 - 5 * width / 32, HorizontalAlignment.CENTER, VerticalAlignment.CENTER);
  }
  ...
  public static enum RectifierType {
    ...
    MiniDIP1("DFM A", PinShape.Square), MiniDIP2("DFM B", PinShape.Square), MiniRound1(
        "Round WOG A", PinShape.Round), MiniRound2("Round WOG B", PinShape.Round), InLine(
            "In-Line D-44", PinShape.Round), SquareBR3("Square BR-3", PinShape.Round);
