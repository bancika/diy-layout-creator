...
@ComponentDescriptor(name = "Zener diode (schematic symbol)", author = "Branislav Stojkovic", category = "Schematics",
...
public class ZenerDiodeSymbol extends AbstractDiodeSymbol {
  ...
  public void drawIcon(Graphics2D g2d, int width, int height) {
    ...
    g2d.drawLine((int) ((width - size) / 2 + size / Math.sqrt(2)), (height - size) / 2, (int) ((width - size) / 2
        + size / Math.sqrt(2) + finSize), (height - size) / 2 - finSize);
    g2d.drawLine((int) ((width - size) / 2 + size / Math.sqrt(2)), (height + size) / 2, (int) ((width - size) / 2
        + size / Math.sqrt(2) - finSize), (height + size) / 2 + finSize);
  }
  ...
  protected void decorateComponentBody(Graphics2D g2d, boolean outlineMode) {
    ...
    g2d.drawPolyline(new int[] {(int) (width / Math.sqrt(2) + bandSize + finSize),
        (int) (width / Math.sqrt(2) + bandSize), (int) (width / Math.sqrt(2) + bandSize),
        (int) (width / Math.sqrt(2) + bandSize - finSize)}, new int[] {(int) -finSize, 0, (int) width,
        (int) (width + finSize)}, 4);
  }
}
