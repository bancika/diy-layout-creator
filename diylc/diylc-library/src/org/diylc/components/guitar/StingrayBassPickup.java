...
public class StingrayBassPickup extends AbstractGuitarPickup {
  ...
  private static Size LIP_DISTANCE_VERTICAL = new Size(0.935d, SizeUnit.in);// Left side vertical between mount points
  ...
  private static Size POLE_SPACING_HORIZONTAL = new Size(0.75366d, SizeUnit.in); // Distance Between poles
  private static Size POLE_SPACING_TO_EDGE= new Size(0.4875d, SizeUnit.in);// Outer edge to centre of nearest pole 
...
//        VerticalAlignment.CENTER);
  ...
  public Shape[] getBody() {
    if (body == null) {
      ...
      lip.subtract(new Area(new Ellipse2D.Double(-lipRadius / 2 + lipHoleSpacing, -lipRadius / 4 + lipHoleSize / 2, lipHoleSize, lipHoleSize)));
      lip.transform(AffineTransform.getTranslateInstance(x - length, y - pointMargin + (width - lipDistanceV) / 2));
      ...
      lip2.subtract(new Area(new Ellipse2D.Double(lipRadius / 2 - lipHoleSize - lipHoleSpacing, -lipRadius / 4 + lipHoleSize / 2, lipHoleSize, lipHoleSize)));
      lip2.transform(AffineTransform.getTranslateInstance(x, y - pointMargin + width / 2));
      lip.add(lip2);
      
      Area lip3 = new Area(new Ellipse2D.Double(-lipRadius / 2, -lipRadius / 2, lipRadius, lipRadius));
      lip3.subtract(new Area(new Ellipse2D.Double(-lipRadius / 2 + lipHoleSpacing, -lipRadius / 4 + lipHoleSize / 2, lipHoleSize, lipHoleSize)));
      lip3.transform(AffineTransform.getTranslateInstance(x - length,  (y - pointMargin) + (width - ((width - lipDistanceV) / 2))));
      lip.add(lip3);
      ...
      // ??? - Perhaps Overall Area - it's never filled in draw()
      ...
      for (int i = 0; i < 4; i++) {
        ...
        Ellipse2D pole =
            new Ellipse2D.Double(x - length + poleMargin + i * poleSpacing - poleSize / 2, 
                                 y + poleSpacingEdge - pointMargin - poleSize / 2 , 
                                 poleSize, poleSize);
        // Bottom Row
        Ellipse2D pole2 = 
            new Ellipse2D.Double(x - length + poleMargin + i * poleSpacing - poleSize / 2, 
  ...
  public void drawIcon(Graphics2D g2d, int width, int height) {
    ...
    g2d.fillRoundRect((width - lipSize) / 2, (height - bodyLength) / 2 - lipSize + 1, lipSize, lipSize, lipSize, lipSize); // Right, center
    g2d.fillRoundRect(9 * width / 32, bodyLength + ((height - bodyLength) / 2), lipSize, lipSize, lipSize, lipSize);
    g2d.fillRoundRect(18 * width / 32, bodyLength + ((height - bodyLength) / 2), lipSize, lipSize, lipSize, lipSize);
    ...
    for (int i = 0; i < 4; i++) {
      // Top Row
      g2d.fillOval(11 * width / 32, 5 * width / 32 + i * poleSpacing, poleSize * width / 32, poleSize * width / 32);
      //g2d.fillOval((width - poleSize) / 2, (height - poleSpacing) / 2 + (i * poleSpacing / 3), poleSize, poleSize);
      // Bottom Row
      g2d.fillOval(17 * width / 32, 5 * width / 32 + i * poleSpacing, poleSize * width / 32, poleSize * width / 32);
    }
  }
