...
public abstract class AbstractNetlistParser implements INetlistParser {
  ...
  public List<ParsedNetlistEntry> parseFile(String fileName, List<String> outputWarnings) throws NetlistParseException {
    String content;
    try {
  ...
  public List<ParsedNetlistEntry> parse(String content, List<String> outputWarnings) throws NetlistParseException {
    ...
    while (matcher.find()) {
      ...
      List<Class<?>> typeCandidates = getComponentTypeCandidates(type, valueMap);
    ...
    Collections.sort(result, new Comparator<ParsedNetlistEntry>() {
      ...
      public int compare(ParsedNetlistEntry o1, ParsedNetlistEntry o2) {
        ...
        if (comp == 0 && o1.getValues().containsKey("Name") && o2.getValues().containsKey("Name"))
          comp = o1.getValues().get("Name").toString().compareToIgnoreCase(o2.getValues().get("Name").toString());        
        return comp;
      }      
    });
  ...
  public List<IDIYComponent<?>> generateComponents(List<ParsedNetlistComponent> components,
    ...
    for (int i = 0; i < components.size(); i++) {
      ...
      for (Map.Entry<String, Object> value : parsedComponent.getValues().entrySet()) {
        try {
          Optional<Method> first = Arrays.stream(methods).filter(x -> x.getName().equals("set" + value.getKey())).findFirst();
          if (first.isPresent()) {
