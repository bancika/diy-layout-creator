...
public class DrawingManager {
	...
	// how much extra space we add around the project area, rounded to grid
	// spacing
	private double extraSpace = 0.25d;
	...
	public DrawingManager(MessageDispatcher<EventType> messageDispatcher) {
		...
		String debugContinuityAreasStr = System
				.getProperty(DEBUG_CONTINUITY_AREAS);
		debugContinuityAreas = debugContinuityAreasStr != null
				&& debugContinuityAreasStr.equalsIgnoreCase("true");
	}
	...
	public List<IDIYComponent<?>> drawProject(Graphics2D g2d, Project project,
		...
		Rectangle2D visibleRect = getVisibleRectangle(project, zoom);
		if (visibleRect != null) {
			g2d.setColor(theme.getOutlineColor());
			g2d.setStroke(ObjectCache.getInstance().fetchStroke(2,
					new float[] { 5f, 10f }, 0f));
			g2d.draw(visibleRect);
			
			g2d.translate(visibleRect.getX(), visibleRect.getY());
		}
		...
		for (IDIYComponent<?> component : project.getComponents()) {
			...
			synchronized (g2d) {
				...
				if (trackArea && area != null
						&& !area.getOutlineArea().isEmpty()) {
					if (visibleRect != null)
						area.translate(visibleRect.getX(), visibleRect.getY());
		...
		if (continuityArea != null
				&& (ConfigurationManager.getInstance().readBoolean(
						IPlugInPort.HIGHLIGHT_CONTINUITY_AREA, false))) {
	...
	public List<IDIYComponent<?>> findComponentsAt(Point point, Project project) {
		...
		for (int i = 0; i < project.getComponents().size(); i++) {
			ComponentArea area = componentAreaMap.get(project.getComponents()
					.get(i));
	...
	public Dimension getCanvasDimensions(Project project, Double zoomLevel,
		...
		int paddingX = (int) (width * extraSpace / project.getGridSpacing()
				.convertToPixels());
		int paddingY = (int) (height * extraSpace / project.getGridSpacing()
				.convertToPixels());

		return new Dimension((int) (width + 2
				* project.getGridSpacing().convertToPixels() * paddingX),
				(int) (height + 2 * project.getGridSpacing().convertToPixels()
						* paddingY));
	}

	private Rectangle2D getVisibleRectangle(Project project, Double zoomLevel) {
		if (extraSpace <= 0)
			return null;
		double width = project.getWidth().convertToPixels();
		double height = project.getHeight().convertToPixels();
		...
		return new Rectangle2D.Double(project.getGridSpacing()
				.convertToPixels() * paddingX, project.getGridSpacing()
				.convertToPixels() * paddingY, width, height);
	}
	...
	public void findContinuityAreaAtPoint(Project project, Point p) {
		...
		for (IDIYComponent<?> c : project.getComponents()) {
			...
			if (type.isContinuity()) {
				connections.add(new Connection(c.getControlPoint(0), c
						.getControlPoint(c.getControlPointCount() - 1)));
			}
	...
	private boolean crunchAreas(List<Area> areas, List<Connection> connections) {
		...
		for (int i = 0; i < areas.size(); i++) {
			for (int j = i + 1; j < areas.size(); j++) {
				...
				// if the two areas intersect, make a union and consume the
				// second area
				if (!intersection.isEmpty()) {
					...
					for (Connection p : connections) {
						if ((a1.contains(p.pointA) && a2.contains(p.pointB))
								|| (a1.contains(p.pointB) && a2
										.contains(p.pointA))) {
							a1.add(a2);
							consumed.set(j, true);
							break;
	...
	private List<Area> tryBreakout(Area a) {
		...
		while (!pathIterator.isDone()) {
			...
			case PathIterator.SEG_CUBICTO:
				p.curveTo(coord[0], coord[1], coord[2], coord[3], coord[4],
						coord[5]);
				break;
