...
public class MainFrame extends JFrame implements ISwingUI {
  ...
  public MainFrame() {
    ...
    presenter.installPlugin(() -> new org.diylc.swing.plugins.guitar.GuitarFeaturesPlugin(this));
  ...
  public boolean editProperties(List<PropertyWrapper> properties, Set<PropertyWrapper> defaultedProperties, String title) {
    PropertyEditorDialog editor =
        DialogFactory.getInstance().createPropertyEditorDialog(properties, title, true);
  ...
  class FramePlugin implements IPlugIn {
    ...
    public void processMessage(EventType eventType, Object... params) {
      if (eventType == EventType.FILE_STATUS_CHANGED) {
        ...
        setTitle(String.format("DIY Layout Creator v%s.%s.%s - %s %s", plugInPort.getCurrentVersionNumber().getMajor(), plugInPort.getCurrentVersionNumber().getMinor(),
