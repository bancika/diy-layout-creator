...
@ComponentDescriptor(name = "Electrolytic Capacitor", author = "Branislav Stojkovic", category = "Passive",
...
public class RadialElectrolytic extends AbstractRadialComponent<Capacitance> {
  ...
  public void drawIcon(Graphics2D g2d, int width, int height) {
    ...
    marker.subtract(new Area(new Rectangle2D.Double(margin, margin, width - 4 * margin, getClosestOdd(width - 2
        * margin))));
  ...
  protected void decorateComponentBody(Graphics2D g2d, boolean outlineMode) {
    ...
    if (polarized) {
      ...
      if (folded) {
        ...
        g2d.drawLine((int) (totalDiameter * (invert ? 0.1 : 0.9)), totalDiameter / 2 - (int) (totalDiameter * 0.05),
            (int) (totalDiameter * (invert ? 0.1 : 0.9)), totalDiameter / 2 + (int) (totalDiameter * 0.05));
      }
    }
