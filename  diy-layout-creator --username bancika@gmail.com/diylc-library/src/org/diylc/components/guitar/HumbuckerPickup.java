...
public class HumbuckerPickup extends AbstractTransparentComponent<String> {
	...
	private static Size LENGTH = new Size(68.4d, SizeUnit.mm);
	...
	private static Size POLE_SIZE = new Size(2.5d, SizeUnit.mm);
	private static Size POLE_SPACING = new Size(9.8d, SizeUnit.mm);
		...
		if (body == null) {
			...
			for (int i = 0; i < 6; i++) {
				Ellipse2D pole = new Ellipse2D.Double(x + pointMargin - length + poleMargin + i
						* poleSpacing, y - pointMargin + coilMargin, poleSize, poleSize);
				poleArea.add(new Area(pole));
				pole = new Ellipse2D.Double(
						x + pointMargin - length + poleMargin + i * poleSpacing, y - pointMargin
								+ width - coilMargin - poleSize, poleSize, poleSize);
				poleArea.add(new Area(pole));
			}
			body[3] = poleArea;
