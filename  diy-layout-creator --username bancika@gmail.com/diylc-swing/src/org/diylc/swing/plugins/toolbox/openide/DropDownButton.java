...
class DropDownButton extends JButton {
	...
	public DropDownButton(Icon icon, JPopupMenu popup) {
		...
		this.resetIcons();
		this.addPropertyChangeListener("dropDownMenu",
		...
		this.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				if (null != DropDownButton.this.getPopupMenu()) {
					DropDownButton.this.mouseInArrowArea = DropDownButton.this
	...
	private boolean isInArrowArea(Point p) {
		return p.getLocation().x >= this.getWidth()
				- IconWithArrow.getArrowAreaWidth() - this.getInsets().right;
	}
	...
	private Icon updateIcons(Icon orig, String iconType) {
		...
		if (null == orig) {
			...
			arrow = new ImageIcon(
					ImageUtilities.icon2Image((Icon) new IconWithArrow(orig,
							false)));
